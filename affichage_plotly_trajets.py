...
def execute_req_google(parameters,
    ...
    if response.status_code != 200:  # see HTTP errors
        ...
        result = json.loads(response.content.decode())['rows'][0]['elements'][0]
...
def affichage_plotlty(nom_ville, minutes_max):
    ...
    for i in range(nombre_aeroports):
        para1 = get_params_google(nom_ville, str(aeroports_recherche['lat'].iloc[i]) + ',' + str(aeroports_recherche['lon'].iloc[i]))
        d1 = execute_req_google(para1)
        code_aeroport = aeroports_recherche['code'].iloc[i]
        print('Recherche des destinations depuis ' + str(code_aeroport) + ' (' + str(i + 1) + '/' + str(
            nombre_aeroports) + ').')
        ...
        airports_dest['duration1'] = d1
        airports_dest['duration2'] = 0
        for j in range(len(airports_dest)):
            para2 = get_params_google(str(airports_dest['lat'].iloc[j]) + ',' + str(airports_dest['lon'].iloc[j]), airports_dest['cityTo'].iloc[j])
            airports_dest['duration2'].iloc[j] = execute_req_google(para2)
    ...
    trajets_train_reduit = trajets_train_copy[
        ['depart', 'station_orig_lat', 'station_orig_lon', 'arrival', 'station_dest_lat', 'station_dest_lon',
         'Tps_trajet_sec', 'total_time', 'Distance', 'mode']]
    ...
    dest_aero_reduit = dest_aero_copy[
        ['cityFrom', 'lat_ori', 'lon_ori', 'cityTo', 'lat', 'lon', 'Tps_total_sec', 'Tps_trajet_sec', 'mode',
         'Distance']]
    dest_reduit_total = dest_aero_reduit.append(trajets_train_reduit)
    dest_reduit_total.loc[len(dest_reduit_total)] = [nom_ville, lat_ori, lon_ori, nom_ville, lat_ori, lon_ori, 0, 0,
                                                     'Origine', 0]
...
def affichage_classique():
    fig = plt.plot(dest_reduit_total, x="lon_prop_tps", y="lat_prop_tps", hover_name='cityTo', color='mode',
                   title="Destinations depuis Bordeaux", kind='scatter')
