...
def affichage_total(nom_recherche):
    ...
    for i in range(nombre_aeroports):
        ...
        for j in range(n):
            ...
            distance = airports_dest['Distance'].iloc[j]
            Tps_trajet = airports_dest['Tps_trajet'].iloc[j]
            ...
            Tps_trajet_sec = airports_dest['Tps_trajet_sec'].iloc[j]
            coef =500*Tps_trajet_sec/7200/distance
            new_coord= cdist.coord_homotethie(airport_ori_longitude,airport_ori_latitude,longitude,latitude,coef)    
            ...
            if (not(pd.isna(new_coord[1])) and not(pd.isna(new_coord[0]))):
                ...
                #Tracé de la ligne
                points=[tuple([airport_ori_latitude,airport_ori_longitude]),tuple([new_coord[1],new_coord[0]])]
